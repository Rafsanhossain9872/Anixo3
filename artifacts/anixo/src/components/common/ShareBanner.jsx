...
const ShareBanner = () => {
  const siteUrl    = window.location.origin || 'https://anixo.online';
  ...
  // Helper: use admin-configured URL if set, else fall back to default share URL
  const link = (key, defaultUrl) => socialLinks[key] || defaultUrl;
  ...
  const shareButtons = [
    {
      ...
      color: 'bg-[#3b5998]',
      count: '63k',
      url: link('facebook', `https://www.facebook.com/sharer/sharer.php?u=${siteUrl}&quote=${shareText}`),
    },
    {
      name: 'Twitter',
      ...
      color: 'bg-[#000000]',
      count: '54.1k',
      url: link('twitter', `https://twitter.com/intent/tweet?text=${shareText}%20${siteUrl}`),
    },
    {
      name: 'Messenger',
      ...
      color: 'bg-[#0084ff]',
      count: '59.7k',
      url: link('messenger', `fb-messenger://share/?link=${siteUrl}`),
    },
    {
      name: 'Reddit',
      ...
      color: 'bg-[#ff4500]',
      count: '90.3k',
      url: link('reddit', `https://www.reddit.com/submit?url=${siteUrl}&title=${shareText}`),
    },
    {
      name: 'WhatsApp',
      ...
      color: 'bg-[#25d366]',
      count: '7.5k',
      url: link('whatsapp', `https://api.whatsapp.com/send?text=${shareText}%20${siteUrl}`),
    },
    {
      name: 'Telegram',
      ...
      color: 'bg-[#0088cc]',
      count: '',
      url: link('telegram', `https://t.me/share/url?url=${siteUrl}&text=${shareText}`),
    },
  ];
  ...
  return (
    <div className="w-full max-w-[1500px] mx-auto px-4 md:px-8 mb-10 mt-6">
      <div className="relative overflow-hidden bg-gradient-to-r from-[#1a1217] via-[#21141e] to-[#271725] border border-white/5 rounded-2xl p-4 md:p-6 lg:px-10 flex flex-col md:flex-row items-center justify-between gap-5 transition-all duration-300 shadow-2xl">
        <div className="absolute top-0 left-0 w-64 h-64 bg-red-600/5 rounded-full blur-3xl pointer-events-none -translate-x-1/2 -translate-y-1/2" />
