...
import {
  Shield, FileText, Users, AlertTriangle, Scale,
  ChevronRight, Globe, Mail, Lock, Eye, Ban, RefreshCw, Gavel,
} from "lucide-react";
import Navbar from "../components/layout/Navbar";
import Footer from "../components/layout/Footer";

// Renders plain-text content from DB (double-newline = paragraph break)
...
export default function TermsOfService() {
  ...
  useEffect(() => {
    ...
    window.scrollTo(0, 0);
    // Fetch admin-editable content from DB; fall back gracefully
    fetch("/api/admin/legal/sections")
      .then((r) => r.json())
      .then((data) => {
        if (Array.isArray(data)) {
          const map = {};
          data.forEach((s) => { if (s.content?.trim()) map[s.sectionId] = s; });
          setDbSections(map);
        }
      })
      .catch(() => {});
  ...
  // ── Hardcoded section definitions — override via Admin Panel ─────────────────
  const sections = [
    {
      id: "acceptance",
      icon: FileText,
      ...
      content: (
        <>
          ...
          <ol className="list-decimal list-inside space-y-2.5 text-white/40 mt-4">
            <li><span className="text-white/50">Reproduce, distribute, or publicly display</span> any content from AniXo without prior written authorization.</li>
            <li><span className="text-white/50">Attempt to circumvent, disable, or interfere</span> with any security features, access controls, or technical measures of the Service.</li>
            <li><span className="text-white/50">Use automated systems</span> (including bots, scrapers, or crawlers) to access, extract, or collect data from the Service without express written permission.</li>
            <li><span className="text-white/50">Transmit or upload</span> any material that contains viruses, malware, or other harmful code that may damage or interfere with the Service.</li>
            <li><span className="text-white/50">Impersonate any person or entity</span>, or falsely state or misrepresent your affiliation with any person or entity.</li>
            <li><span className="text-white/50">Use the Service for any commercial purpose</span> or for the benefit of any third party without our prior written consent.</li>
            <li><span className="text-white/50">Engage in any activity</span> that could disable, overburden, or impair the proper functioning of the Service or its underlying infrastructure.</li>
          </ol>
          <p className="mt-4">
    ...
    // ── Section 05 — Intellectual Property Rights (+ NON-LIABILITY CLAUSE) ────
    {
      ...
      content: (
        <>
          ...
          <div className="mt-5 p-4 bg-white/[0.03] border border-white/10 rounded-lg">
            <p className="text-[12px] text-white/50 font-semibold uppercase tracking-widest mb-2">
              Critical Disclaimer — Non-Liability for Third-Party Content
            </p>
            <p className="text-[13px] text-white/40 leading-relaxed">
              AniXo operates exclusively as a public link-indexing aggregator. It does <strong className="text-white/60">not</strong> host,
              upload, reproduce, cache, store, stream, or transmit any video binary content, media files, audio streams,
              or copyrighted audiovisual works on its own servers, databases, or content delivery infrastructure.
              All video content accessible through AniXo is hosted, stored, and streamed directly from independent
              third-party servers wholly unaffiliated with AniXo. AniXo's technical function is strictly limited to indexing
              publicly available URLs and providing navigational hyperlinks to content hosted on external servers —
              no copy of any third-party audiovisual work is ever placed, buffered, or processed on AniXo's infrastructure
              at any point in the content delivery chain. Accordingly, AniXo bears no copyright ownership, licensing obligation,
              or structural copyright liability for any third-party audiovisual content linked through its interface.
            </p>
          </div>
        </>
      ),
    ...
    // ── Section 06 — Third-Party Content Disclaimer (+ STRUCTURAL LIABILITY EXEMPTION) ──
    {
      id: "third-party",
      icon: Globe,
      ...
      content: (
        <>
          ...
          <div className="mt-5 p-4 bg-white/[0.03] border border-white/10 rounded-lg">
            <p className="text-[12px] text-white/50 font-semibold uppercase tracking-widest mb-2">
              Structural Liability Exemption
            </p>
            <p className="text-[13px] text-white/40 leading-relaxed">
              AniXo's technical architecture is that of a pure aggregation and search-indexing service.
              It does not place, store, reproduce, cache, or transmit any copy of audiovisual works on its own servers
              or CDN infrastructure at any point in the content delivery chain. All media streaming, buffering, and
              delivery occurs exclusively between the user's browser or device and the independent third-party origin
              servers where such content is physically stored and served. AniXo's role is strictly limited to surfacing
              publicly accessible hyperlinks — functionally equivalent to the indexing and linking operations of a
              conventional web search engine. AniXo cannot therefore be held structurally liable for the content,
              continued availability, accuracy, or legality of third-party materials linked through its interface,
              as it exercises no technical control over the storage, transmission, encoding, or availability of
              such content at any stage of delivery.
            </p>
          </div>
    ...
    // ── Section 07 — DMCA Compliance (+ SAFE HARBOR PROTECTION) ─────────────
    {
      id: "dmca",
      icon: Shield,
      ...
      content: (
        <>
          ...
          <div className="mt-5 p-4 bg-white/[0.03] border border-white/10 rounded-lg">
            <p className="text-[12px] text-white/50 font-semibold uppercase tracking-widest mb-2">
              DMCA Safe Harbor Protection — 17 U.S.C. § 512
            </p>
            <p className="text-[13px] text-white/40 leading-relaxed">
              AniXo qualifies for and expressly claims the statutory protections afforded by 17 U.S.C. § 512
              of the Digital Millennium Copyright Act. Operating as an information location tool that indexes and links
              to publicly available content without hosting, reproducing, caching, or transmitting any copyrighted binary
              content on its own servers or infrastructure, AniXo's operations are fully consistent with the safe harbor
              provisions under § 512(d) for qualifying service providers who provide information location tools such as
              directories, indexes, and hyperlinks. AniXo: (1) does not have actual knowledge of specific infringing
              material residing on third-party servers; (2) is not aware of facts or circumstances from which infringing
              activity is apparent; (3) does not receive a direct financial benefit from infringing activity over which it
              exercises any right or ability to control; (4) acts expeditiously upon receipt of a valid, DMCA-compliant
              takedown notice to remove or disable access to the linked content; and (5) has designated a DMCA agent with
              the United States Copyright Office as required by statute. All video streams and media content are delivered
              exclusively from independent third-party origin servers over which AniXo exercises no operational control,
              placing AniXo squarely within the statutory definition of an eligible "service provider" entitled to DMCA
              safe harbor protection under both § 512(d) and § 512(a) as applicable.
            </p>
          </div>
        </>
      ),
      ...
      <main className="max-w-[900px] mx-auto px-4 md:px-8 pt-28">
        ...
        <div className="mb-12 p-5 bg-white/[0.02] border border-white/5 rounded-xl">
          ...
          <nav className="space-y-2.5 columns-1 sm:columns-2">
            ...
            <a href="#contact-info" className="flex items-center gap-3 text-[13px] font-normal text-white/35 hover:text-white/70 transition-all group break-inside-avoid">
        ...
        <div className="space-y-14">
          {sections.map((section, i) => {
            ...
            const dbOverride = dbSections[section.id];
            return (
          ...
          <section id="contact-info" className="scroll-mt-28">
            ...
            <div className="pl-11">
              {dbSections["contact"]?.content?.trim() ? (
                <div className="text-[13px] md:text-[14px] text-white/40 leading-[1.85] font-normal space-y-4">
                  {renderText(dbSections["contact"].content)}
                </div>
              ) : (
                <>
