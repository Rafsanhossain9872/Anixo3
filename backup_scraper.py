...
# Load environment variables (python-dotenv is optional)
try:
...
def api_stream():
    """
    Video stream fallback endpoint.
    Called by the Node.js S1/S2 routes when MegaPlay is unreachable.
    Returns: { success: true, sources: [{ url, type }] }
    """
    anime_id = request.args.get("id", "").strip()
    episode   = request.args.get("episode", "1").strip()
    lang      = request.args.get("lang", "sub").strip().lower()
    id_type   = request.args.get("type", "ani").strip().lower()  # "ani" or "mal"

    if not anime_id:
        return {"error": "Missing id"}, 400

    megabase = os.environ.get("VITE_MEGAPLAY_URL", "https://megaplay.buzz")
    url = f"{megabase}/stream/{id_type}/{anime_id}/{episode}/{lang}"

    log.info(f"[Python Fallback] Attempting stream for {id_type}/{anime_id} ep={episode} lang={lang}")

    try:
        # Use cloudscraper to bypass Cloudflare protection on megaplay
        resp = http.get(url, timeout=15)
        resp.raise_for_status()
        log.info(f"[Python Fallback] ✅ MegaPlay reachable at {url}")
        return {
            "sources": [{"url": url, "type": "iframe"}],
            "server": f"python-megaplay-{id_type}",
            "lang": lang,
        }
    except Exception as e:
        log.warning(f"[Python Fallback] MegaPlay unreachable: {e}")

    # Last-resort: return the URL anyway so the player can try
    log.info("[Python Fallback] Returning iframe URL as last resort")
    return {
        "sources": [{"url": url, "type": "iframe"}],
        "server": "python-fallback-lastresort",
        "lang": lang,
        "warning": "Stream availability unverified",
    }
...
if __name__ == "__main__":
    ...
    # Default port 5000 for local fallback; 7860 for HuggingFace Spaces
